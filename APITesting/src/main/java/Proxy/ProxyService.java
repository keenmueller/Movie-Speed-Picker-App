...
//            loginResponse.setMessage("ERROR: " + conn.getResponseMessage());
    ...
    public DetailResponse getDetails(DetailRequest r, String urlString) throws IOException{
        URL url = new URL(urlString);
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        conn.setRequestMethod("POST");
        conn.setDoOutput(true);
        ...
        if (conn.getResponseCode() == HttpURLConnection.HTTP_OK) {
            InputStream resBody = conn.getInputStream();
            String response = readString(resBody);
            DetailResponse detailResponse = gson.fromJson(response, DetailResponse.class);
